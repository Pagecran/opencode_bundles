...
bool FPagecranBridgeServer::Start(const FString& InHost, int32 InPort)
{
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("PagecranUnrealBridge: invalid host '%s'"), *InHost);
        return false;
    }
    ...
    if (!Listener.IsValid())
    {
        UE_LOG(LogTemp, Error, TEXT("PagecranUnrealBridge: failed to create TCP listener"));
        return false;
    }
    ...
    UE_LOG(LogTemp, Display, TEXT("PagecranUnrealBridge listening on %s:%d"), *Host, Port);
    return true;
}
