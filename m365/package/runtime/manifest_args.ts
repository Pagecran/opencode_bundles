...
import { type MethodArgManifest, type MethodManifest } from "./method_registry"
...
function validateEnum(value: unknown, argManifest: MethodArgManifest, argPath: string) {
  ...
  if (!enumValues.includes(String(value))) {
    throw new Error(
      `Invalid value for ${argPath}. Expected one of: ${enumValues.join(", ")}.`
    )
  }
}
...
export function validateAndNormalizeArgs(manifest: MethodManifest, rawArgs: JsonRecord = {}) {
  ...
  const knownArgNames = new Set(Object.keys(manifest.args))
  ...
  for (const [argName, argManifest] of Object.entries(manifest.args)) {
